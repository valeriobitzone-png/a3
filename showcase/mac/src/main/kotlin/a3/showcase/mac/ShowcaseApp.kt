...
import a3.renderers.mac.compose.MacTheme
import a3.renderers.mac.compose.ParticleBurst
import a3.showcase.CalendarFixture
import a3.showcase.LoggingSink
import a3.showcase.ShowcaseChromeMath
...
fun ShowcaseApp(
    ...
    val paper = argb(tokens.paper)
    val ink = argb(tokens.ink)
    ...
    Column(
        ...
        Box(Modifier.weight(1f).fillMaxWidth().testTag("showcase-scene")) {
            ...
            if (flags.ambient) {
                ShowcaseAmbient(expanded = ambientExpanded, reduced = reduced || staticChrome, paper = paper)
            }
...
private fun Controls(
    ...
    val style = MacTheme.type.copy(fontSize = 13.sp, color = ink)
...
private fun Chip(label: String, tag: String, active: Boolean, ink: Color, onClick: () -> Unit) {
    val amber = argb(ShowcaseTokens.snapshot.amber)
    Box(
        Modifier
            .testTag(tag)
        ...
        BasicText(label, style = MacTheme.type.copy(fontSize = 13.sp, color = ink))
    }
}
    ...
    Canvas(Modifier.fillMaxSize().testTag("animated-gradient")) {
        ...
        while (y < size.height.toInt()) {
            ...
            while (x < size.width.toInt()) {
                val packed = ShowcaseChromeMath.shade(x / size.width, y / size.height, t, pal)
                drawRect(
                    color = argb(packed),
...
internal fun ShowcaseParticles(trigger: Boolean, staticChrome: Boolean) {
    ...
    Canvas(Modifier.fillMaxSize().testTag(if (trigger) "particle-canvas" else "particle-canvas-idle")) {
        for (spark in sparks) {
            drawCircle(
                color = argb(ShowcaseTokens.snapshot.amber).copy(alpha = spark.life),
...
@Composable
internal fun ShowcaseAmbient(expanded: Boolean, reduced: Boolean, paper: Color) {
    val tokens = ShowcaseTokens.snapshot
    ...
    Box(
        ...
        BasicText("12:00", style = MacTheme.type.copy(fontSize = 12.sp, color = paper))
    }
}
...
internal fun ShowcaseModal(open: Boolean, reduced: Boolean) {
    ...
    Box(Modifier.fillMaxSize().zIndex(5f).testTag("modal-surface")) {
        Box(Modifier.fillMaxSize().testTag("modal-backdrop"))
        Box(
            Modifier
                .fillMaxSize()
                .testTag("modal-scrim")
        ...
        Box(Modifier.align(Alignment.Center).padding(24.dp).testTag("modal-body")) {
            BasicText("confirm", style = MacTheme.type.copy(color = Color.White))
        }
    }
}

private fun argb(color: Int): Color = Color(
