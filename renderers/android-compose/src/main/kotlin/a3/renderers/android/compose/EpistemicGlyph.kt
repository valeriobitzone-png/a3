...
internal fun EpistemicGlyph(node: RenderedNode) {
    ...
    val kind = GlyphGeometry.kind(axis.action.name.lowercase(), axis.status.name.lowercase())
        ?: return
...
internal fun glyphKind(axis: EpistemicAxis): GlyphGeometry.Kind? =
    GlyphGeometry.kind(axis.action.name.lowercase(), axis.status.name.lowercase())
